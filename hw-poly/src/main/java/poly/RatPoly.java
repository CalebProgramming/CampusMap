...
public final class RatPoly {
    ...
    private RatPoly(List<RatTerm> rt) {
        this();
        // {inv: terms = {rt[0]+...rt[i-1]}}
        for (int i = 0; i != rt.size(); i++) {
            sortedInsert(terms, rt.get(i));
        }
    ...
    private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
        ...
        if (!newTerm.getCoeff().equals(RatNum.ZERO)) {
            boolean flag = false;
            int i = 0;
            //{inv: lst[0,...,i-1] is sorted and newTerm hasn't inserted in}
            while (i != lst.size() && !flag) {
                RatTerm ratTerm = lst.get(i);
                if (ratTerm.getExpt() < newTerm.getExpt()) {
                    lst.add(i, newTerm);
                    flag = true;
                } else if (ratTerm.getExpt() == newTerm.getExpt()) {
                    if (newTerm.equals(ratTerm.negate())) {
                        lst.remove(i);
                    } else {
                        lst.set(i, ratTerm.add(newTerm));
                    }
                    flag = true;
                }
                i++;
    ...
    public RatPoly add(RatPoly p) {
        ...
        //{inv: list = list + p.terms[0,...i-1]}
        for (int i = 0; i != p.terms.size(); i++) {
            sortedInsert(list, p.terms.get(i));
            if (p.terms.get(i).isNaN()) {
                return NaN;
            }
        }
        return new RatPoly(list);
    ...
    public RatPoly mul(RatPoly p) {
        ...
        if (p.terms.size() == 0 || terms.size() == 0) {
            return ZERO;
        }
        //{inv: list = terms[0,...,i-1] * p.terms}
        for (int i = 0; i != terms.size(); i++) {
            //{inv: list = terms[0,...i-2] * p.terms + terms[i-1] * p.terms[0,...,j-1]}
            for (int j = 0; j < p.terms.size(); j++) {
                if (terms.get(i).isNaN() || p.terms.get(j).isNaN()) {
                    return NaN;
                } else {
                    sortedInsert(list, terms.get(i).mul(p.terms.get(j)));
                }
            }
        }
        return new RatPoly(list);
    ...
    public RatPoly div(RatPoly p) {
        ...
        RatPoly rm = new RatPoly(terms);
        // {inv: this = rm + r * q}
        while (rm.degree() >= p.degree() && !rm.equals(ZERO)) {
            RatPoly t_r;
            if (rm.degree() - p.degree() == 0) {
                t_r = new RatPoly(new RatTerm(rm.getTerm(rm.degree()).getCoeff().div(p.getTerm(p.degree()).getCoeff()), 0));
            } else {
                t_r = new RatPoly(new RatTerm(rm.getTerm(rm.degree()).getCoeff().div(p.getTerm(p.degree()).getCoeff()), rm.degree() - p.degree()));
            }
            r = r.add(t_r);
            rm = rm.sub(p.mul(t_r));
        }
    ...
    public double eval(double d) {
        ...
        Double result = 0.00;
        //{inv: result = value with d of [terms[0]+...terms[i-1]]}
        for (int i = 0; i < terms.size(); i++) {
            result = result + terms.get(i).eval(d);
        }
        return result;
    }
